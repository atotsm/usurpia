// Usurpia Lens Bookmarklet - V1.5 (The "Smart Cap" Model)
// This version implements a maximum highlight cap to prevent the "Christmas Tree Effect"
// while still searching for all known terms to guarantee a high hit-rate on first use.
...
(function() {
    ...
    const config = {
        settings: {
            // The maximum number of highlights to show on a single page to prevent overwhelm.
            maxHighlights: 15,
            depth: 'summary'
        },
        dictionary: [
            ...
            {
                terms: ["economic growth", "growth"], primaryTerm: "economic growth",
            ...
            {
                terms: ["Federal Reserve", "The Fed"], primaryTerm: "Federal Reserve",
            ...
            {
                terms: ["jobs", "job creation"], primaryTerm: "jobs",
            ...
            {
                terms: ["human resources", "human capital"], primaryTerm: "human resources",
    ...
    function highlightKeywords() {
        ...
        // Step 1: Find ALL potential highlight candidates
        let candidates = [];
        let textNodes = [];
        ...
        textNodes.forEach(node => {
            ...
            text.replace(masterRegex, (match, offset) => {
                candidates.push({ node: node, match: match, offset: offset });
            });
        });

        // Step 2: If we exceed the cap, randomly select which ones to highlight.
        let highlightsToPerform = candidates;
        if (candidates.length > config.settings.maxHighlights) {
            // Fisher-Yates shuffle algorithm to randomize the candidates
        ...
        // Step 3: Perform the highlighting only on the selected nodes
        // Group by node to avoid splitting the same node multiple times incorrectly
        const groupedByNode = highlightsToPerform.reduce((acc, {node, match, offset}) => {
            if (!acc[node.nodeValue]) acc[node.nodeValue] = {node, matches: []};
            acc[node.nodeValue].matches.push({match, offset});
            return acc;
        }, {});
        ...
        Object.values(groupedByNode).forEach(({node, matches}) => {
            ...
            matches.forEach(({match, offset}) => {
                if (offset < lastIndex) return; // Skip overlapping matches
